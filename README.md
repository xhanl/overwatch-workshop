# <img src="images/extension/icon.png" width="50" height="50" align=center width=500 /> **Overwatch®** Workshop
...
- 技术限制
  ...
  - 语法解析只支持工坊导入时接受的语法，并以工坊导出时的格式化风格为准
    - 花括号 `{` `}` 应该始终单独成行，行尾花括号等非标准写法导致解析不准确
    - 工坊导出文本和导入语法要求有些差异，例如 `无` 需要改为 `全部禁用`

- 商业限制
  - 禁止用于强制赞助收费项目
  - 允许定制非盈利代码业务（请作者仔细询问定制用途，不要承接用于面向最终玩家收费的项目）
  - 允许非强制赞助（所有玩家都能够免费，平等的体验全部功能, 玩家自发赞助）
  - 如果违反了限制（特别是使用了混淆功能），我们保留将你拉黑，并在技术上限制和追究的权利
