...
function getDynamicList(
  ...
  function calculateRange(
    ...
    const endPos = new vscode.Position(
      lineIndex,
      targetStartIndex + targetName.length,
    );
    ...
    if (!usedGlobalVariables.has(defaultName)) {
      for (let i = 0; i < 128; i++) {
        if (!globalVariables[i]) {
          usedGlobalVariables.add(defaultName);
          globalVariables[i] = { name: defaultName };
          break;
        }
      }
    }
    ...
    if (!usedPlayerVariables.has(defaultName)) {
      for (let i = 0; i < 128; i++) {
        if (!playerVariables[i]) {
          usedPlayerVariables.add(defaultName);
          playerVariables[i] = { name: defaultName };
          break;
        }
      }
    }
  ...
  // 对于子程序，按Sub0-Sub7顺序填入未被用户占用的默认名称
  for (let i = 0; i < 8; i++) {
    const defaultSubName = `Sub${i}`;
    if (!subroutines[i] && !usedSubroutines.has(defaultSubName)) {
      usedSubroutines.add(defaultSubName);
      subroutines[i] = { name: defaultSubName };
    }
  }
...
function getEntry(
  ...
  for (let i = position.line; i >= 0; i--) {
    ...
    //扫描当前行
    const lastCharacter = i === position.line
      ? position.character
                  ...
                  if (element.类型 === "自定义字符串") {
                    ...
                    if (match[2]) {
                      ...
                      for (const paramIndex in paramParams) {
                        let paramMatch;
                        if (
                          (paramMatch = paramParams[paramIndex].match(
                            /"([^"]*)"/,
                          ))
                        ) {
                          insertText.value += '"${' +
